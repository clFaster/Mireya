...
public class PlaybackReportingServiceTests
{
    ...
    private static Screen SeedScreen(TestDatabase db, string userId, string name)
    {
    ...
    public async Task RecordAsync_PersistsPlayForKnownScreen()
    {
        ...
        SeedScreen(db, "screen-1", "Front Window");
        ...
        Assert.Equal("Front Window", entry.ScreenName);
    ...
    public async Task GetReportAsync_AggregatesByAssetAndScreen()
    {
        using var db = new TestDatabase();
        SeedScreen(db, "screen-1", "Front Window");
        SeedScreen(db, "screen-2", "Back Wall");
        var service = CreateService(db);
        ...
        Assert.Equal("Front Window", report.ByScreen[0].ScreenName);
    ...
    public async Task GetReportAsync_ExcludesEventsOutsideWindow()
    {
        ...
        db.Context.PlaybackEvents.Add(
            new PlaybackEvent
            {
                ScreenId = db.Context.Screens.First().Id,
                ScreenName = "Front Window",
