...
public class AssetController(IAssetService assetService) : ControllerBase
{
    ...
    public async Task<ActionResult<List<AssetSummary>>> UploadAssets([FromForm] UploadFilesRequest request)
    {
        try
        {
    ...
    [HttpGet]
    public async Task<ActionResult<PagedAssets>> GetAssets([FromQuery] int page = 1, [FromQuery] int pageSize = 10, [FromQuery] AssetType? type = null, [FromQuery] string sortBy = "name")
    {
    ...
    public async Task<ActionResult<Asset>> UpdateAssetMetadata(Guid id, [FromBody] UpdateAssetMetadataRequest request)
    {
        try
        {
