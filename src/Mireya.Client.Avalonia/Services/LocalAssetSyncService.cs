...
public interface ILocalAssetSyncService
{
    ...
    Task SyncCampaignsAsync(List<CampaignSyncInfo> campaigns, CancellationToken cancellationToken = default);
...
public class LocalAssetSyncService : ILocalAssetSyncService
{
    ...
    public LocalAssetSyncService(
        ...
        Microsoft.Extensions.Options.IOptions<Mireya.ApiClient.Options.MireyaApiClientOptions> options,
    ...
    public async Task SyncCampaignsAsync(List<CampaignSyncInfo> campaigns, CancellationToken cancellationToken = default)
    {
        ...
        _logger.LogInformation("Syncing campaigns for backend {BackendId} - {BaseUrl}", 
            backend.Id, backend.BaseUrl);
        
        foreach (var campaign in campaigns)
        {
            _logger.LogInformation("Campaign: {CampaignId} - {CampaignName} with {AssetCount} assets",
        ...
        foreach (var asset in uniqueAssets)
        {
            ...
            try
            {
                _logger.LogInformation("Processing asset {AssetId} - {AssetName} ({Type})", 
        ...
        _logger.LogInformation("=== SYNC COMPLETE === Downloaded: {Downloaded}, Skipped: {Skipped}, Errors: {Errors}",
    ...
    {
        _logger.LogDebug("Upserting campaign {CampaignId} - {CampaignName} for backend {BackendId}", 
        ...
        var backendCampaign = await _db.BackendCampaigns
            .FirstOrDefaultAsync(bc => bc.BackendInstanceId == backendId && bc.CampaignId == campaign.CampaignId);
        ...
        foreach (var assetInfo in campaign.Assets)
        {
            ...
            var backendAsset = await _db.BackendAssets
                .FirstOrDefaultAsync(ba => ba.BackendInstanceId == backendId && ba.AssetId == assetInfo.AssetId);
        ...
        var existing = await _db.CampaignAssets
            .Where(ca => ca.CampaignId == campaign.CampaignId)
            .ToListAsync();
        ...
        for (int i = 0; i < campaign.Assets.Count; i++)
        {
        ...
        await _db.SaveChangesAsync();
        _logger.LogInformation("Upserted campaign {CampaignId} with {AssetCount} assets for backend {BackendId}", 
            campaign.CampaignId, campaign.Assets.Count, backendId);
    }

    private async Task<bool> SyncAssetAsync(AssetDownloadInfo asset, Guid backendId, CancellationToken cancellationToken)
    {
        _logger.LogDebug("SyncAssetAsync: Checking asset {AssetId} for backend {BackendId}", asset.AssetId, backendId);
        
        // Check if asset already downloaded for this backend
        var downloadedAsset = await _db.DownloadedAssets
            .FirstOrDefaultAsync(da => da.BackendInstanceId == backendId && da.AssetId == asset.AssetId);
        
        if (downloadedAsset?.IsDownloaded == true && !string.IsNullOrEmpty(downloadedAsset.LocalPath) && File.Exists(downloadedAsset.LocalPath))
        {
            _logger.LogInformation("Asset {AssetId} already downloaded for backend {BackendId} at {Path}, skipping", 
                asset.AssetId, backendId, downloadedAsset.LocalPath);
        ...
        if (downloadedAsset?.IsDownloaded == true && !string.IsNullOrEmpty(downloadedAsset.LocalPath) && !File.Exists(downloadedAsset.LocalPath))
        {
            _logger.LogWarning("Asset {AssetId} marked as downloaded but file missing at {Path}, re-downloading",
                asset.AssetId, downloadedAsset.LocalPath);
        }

        // Website assets don't need downloading
        if (asset.Type == "Website")
        {
            _logger.LogInformation("Asset {AssetId} is a Website, no download needed", asset.AssetId);
        ...
        try
        {
            _logger.LogInformation("Downloading asset {AssetId} from {Source} for backend {BackendId}", 
                asset.AssetId, asset.Source, backendId);
            OnSyncProgressChanged?.Invoke(asset.AssetId, "Downloading", 0);

            var (localPath, extension) = await DownloadAssetAsync(asset, backendId, cancellationToken);
            
            _logger.LogInformation("Successfully downloaded asset {AssetId} to {Path} ({Extension}) for backend {BackendId}", 
                asset.AssetId, localPath, extension, backendId);
        ...
        {
            _logger.LogError(ex, "Failed to download asset {AssetId} for backend {BackendId}", asset.AssetId, backendId);
    ...
    private async Task<(string localPath, string extension)> DownloadAssetAsync(AssetDownloadInfo asset, Guid backendId, CancellationToken cancellationToken)
    {
        _logger.LogDebug("DownloadAssetAsync: Starting download for {AssetId} (backend {BackendId})", 
            asset.AssetId, backendId);
        ...
        if (extension == ".jpg" || extension == ".mp4")
        {
            ...
            if (detectedExtension != null)
            {
                ...
                _logger.LogInformation("Extension updated from Content-Type: {Extension}, new path: {Path}", 
                    extension, localPath);
            }
        }
        ...
        while ((bytesRead = await contentStream.ReadAsync(buffer, cancellationToken)) > 0)
        {
            ...
            if (totalBytes > 0)
            {
                ...
                {
                    _logger.LogInformation("Download progress: {Progress}% ({Downloaded}/{Total} bytes)", 
    ...
    private async Task UpsertDownloadedAssetAsync(Guid backendId, Guid assetId, string? localPath, string? extension, bool isDownloaded)
    {
        var downloadedAsset = await _db.DownloadedAssets
            .FirstOrDefaultAsync(da => da.BackendInstanceId == backendId && da.AssetId == assetId);
        ...
        {
            if (localPath != null) downloadedAsset.LocalPath = localPath;
            if (extension != null) downloadedAsset.FileExtension = extension;
            downloadedAsset.IsDownloaded = isDownloaded;
            if (isDownloaded) downloadedAsset.DownloadedAt = DateTime.UtcNow;
            downloadedAsset.LastCheckedAt = DateTime.UtcNow;
    ...
    private async Task UpdateServerSyncStatusAsync(Guid assetId, string syncState, int progress, string? errorMessage = null)
    {
        try
        {
    ...
    public async Task<List<Guid>> GetMissingAssetIdsAsync(List<Guid> requiredAssetIds)
    {
        ...
        var downloaded = await _db.DownloadedAssets
            .Where(a => a.BackendInstanceId == backend.Id && 
    ...
    public async Task<bool> IsAssetDownloadedAsync(Guid assetId)
    {
        ...
        var downloadedAsset = await _db.DownloadedAssets
            .FirstOrDefaultAsync(da => da.BackendInstanceId == backend.Id && da.AssetId == assetId);
    ...
    public string GetAssetLocalPath(Guid assetId)
    {
        ...
        var downloadedAsset = _db.DownloadedAssets
            .FirstOrDefault(da => da.BackendInstanceId == backend.Id && da.AssetId == assetId);
