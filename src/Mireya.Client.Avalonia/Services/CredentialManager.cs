...
{
    Task SaveCredentialsAsync(Guid backendInstanceId, string username, string accessToken, string? refreshToken = null, DateTime? expiresAt = null);
...
public class CredentialManager : ICredentialManager
{
    ...
    {
        _logger.LogInformation("Saving credentials for backend {BackendId}, username: {Username}", 
            backendInstanceId, username);
    ...
    public async Task<bool> HasValidCredentialsAsync(Guid backendInstanceId)
    {
        ...
        {
            _logger.LogDebug("Credentials exist but access token is empty for backend {BackendId}", backendInstanceId);
            return false;
        }
            
        // Check if token is expired
        if (credential.TokenExpiresAt.HasValue && credential.TokenExpiresAt.Value <= DateTime.UtcNow)
        {
            _logger.LogWarning("Token expired for backend {BackendId} at {ExpiresAt}", 
                backendInstanceId, credential.TokenExpiresAt);
            return false;
        }
    ...
    public async Task DeleteCredentialsAsync(Guid backendInstanceId)
    {
        ...
        {
            _logger.LogDebug("No credentials found to delete for backend {BackendId}", backendInstanceId);
        }
    }
}
