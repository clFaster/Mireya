...
public class AssetSyncService : IAssetSyncService
{
    ...
    )
    {
        ...
        foreach (var campaign in campaigns)
        {
            ...
            catch (Exception ex)
            {
                _logger.LogError(
                    ex,
                    "Failed to sync campaign {CampaignId} ({CampaignName})",
                    campaign.CampaignId,
    ...
    private async Task SyncCampaignAsync(
        CampaignSyncInfo campaign,
        CancellationToken cancellationToken
    )
    {
        _logger.LogInformation(
            "Syncing campaign {CampaignId} ({CampaignName}) with {AssetCount} assets",
            campaign.CampaignId,
            campaign.CampaignName,
    ...
    )
    {
        try
        {
            ...
            _logger.LogDebug(
                "Updated sync status: Asset {AssetId}, State {State}, Progress {Progress}%",
                assetId,
                syncState,
                progress
            );
